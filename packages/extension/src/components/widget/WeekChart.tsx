...
export function WeekChart({ days }: WeekChartProps) {
  ...
  const sortedDays = [...days].sort((a, b) => a.date.localeCompare(b.date)).slice(-7);
  ...
  const maxMinutes = Math.max(...weekDays.map((d) => (d ? Math.round((d.totalSeconds || 0) / 60) : 0)), 1);

  const dayLabels = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];

  return (
    <div className="space-y-1">
      <div className="text-xs text-muted-foreground text-center mb-2">Last 7 Days</div>
        ...
        {weekDays.map((day, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center flex-1">
              <div
                className={`w-full rounded-sm transition-all ${isToday ? 'bg-primary' : 'bg-secondary'}`}
